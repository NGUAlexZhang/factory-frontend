<template>
    <div>
        ...
        <el-dialog :visible.sync="isDelete" title="警告">
            ...
            <div>
                <el-button type="danger" @click="deleteStaff(deleteId)">确定</el-button>
        ...
        <el-dialog :visible.sync="edit" title="编辑">
            <el-form>
                ...
                <el-form-item label="出勤率">
                    <el-input v-model="attend"></el-input>
                </el-form-item>
                <el-form-item>
...
<script>
import {deleteStaff, getStaff, putStaff} from "@/api/staff";
...
export default {
    ...
    methods: {
        async init() {
            const res = await getStaff(this.id, this.name, this.order, this.by, this.page, this.size)
            this.data = res.data
        ...
        async putCustomer() {
            try {
                ...
                console.log(this.date)
                await putStaff(this.editId, this.editName, this.date, this.editSex,this.attend)
                this.$emit('update')
                this.edit = false
            } catch (err) {
                console.error(err)
